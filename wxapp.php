...
class Zh_cjdiancModuleWxapp extends WeModuleWxapp {
    ...
    public function doPagefx_tel(){
        ...
        $res = pdo_get('cjdc_user',array('user_tel'=>$_GPC['fx_tel']));

        if($res){
            //查询分销审核
            $res2 = pdo_get('cjdc_retail',array('user_tel'=>$res['user_tel']));

            if($res2 ==''){
                //更新分销审核
                $ins = pdo_insert('cjdc_retail',array('user_id'=>$res['id'],'time'=>time(),'user_name'=>$res['user_name'],'user_tel'=>$res['user_tel'],'state'=>'2','uniacid'=>$_W['uniacid'],'sh_time'=>time()));
                //更新分销ID

                $fxuser = pdo_get('cjdc_fxuser',array('user_id'=>$_GPC['user_id']));

                if($fxuser ==''){
                    if($ins){
                        $re = pdo_insert('cjdc_fxuser',array('user_id'=>$_GPC['user_id'],'fx_user'=>$res['id'],'time'=>date('Y-m-d H:i:s')));
                    }
                }
            }
        }
    ...
    public function doPageLogin() {
        ...
        if ($res) {
            ...
            if($_GPC['fx_tel']){
                $data['fx_tel'] = $_GPC['fx_tel'];
            }
            $res = pdo_update('cjdc_user', $data, array('id' => $user_id));
            $user = pdo_get('cjdc_user', array('openid' => $openid));
    ...
    public function doPageDishesType() {
        ...
        $type = pdo_getall('cjdc_type', array('isKtvType'=>0,'uniacid' => $_W['uniacid'], 'store_id' => $_GPC['store_id'], 'is_open' => 1), array(), '', 'order_by ASC');
    ...
    public function doPageDishesList() {
        ...
        $isKtvTable = 0;
        if($tableid) {
            $tableRes =  pdo_get('cjdc_table', array('id' => $tableid));
            if($tableRes&&$tableRes['isKtvTable']==1){
                $isKtvTable = 1;
    ...
    public function doPageHot() {
        ...
        $res = pdo_getall('cjdc_goods', array('isKtvGood' => $isKtvTable,'store_id' => $_GPC['store_id'], 'uniacid' => $_W['uniacid'], 'is_hot' => 1, 'is_show' => 1, 'type !=' => $_GPC['type']), array(), '', 'num asc');
        echo json_encode($res);
    }
...
//余额支付
                if ($_GPC['money'] > 0) {
                    ...
                    $money = $user['wallet'] - $_GPC['money'];
                    $int = pdo_update('zh_jdgjb_user',array('balance'=>$money),array('tel'=>$user['user_tel']));
                    //更新商城余额
                    $int2 = pdo_update('ewei_shop_member',array('credit2'=>$money),array('mobile'=>$user['user_tel']));
...
//余额支付
    ...
    public function doPageDmOrder() {
        ...
        $data['uniacid'] = $_W['uniacid'];
        $data['time'] = date('Y-m-d H:i:s');
        $data['selectType'] = $_GPC['selectType'];
        $data['dm_state'] = 1;
        $data['code'] = time() . rand(1000, 9999) . $_GPC['user_id']; //code
        $res = pdo_insert('cjdc_order', $data);
        $order_id = pdo_insertid();
        if ($res) {
            if ($_GPC['pay_type'] == 2) {
                ...
                $user = pdo_get('cjdc_user', array('id' => $res['user_id']));
                $style = "\n";
                $content = $style;
                ...
                if ($selectType < 3) {
                    ...
                    $dyj = pdo_get('zh_jdgjb_dyj', array('id' => $selectType));
                    $rst = Dyj::ylydy($dyj['api'], $dyj['token'], $dyj['yy_id'], $dyj['mid'], $content);

                } else if ($selectType == 3) {
                    $dyj2 = pdo_get('cjdc_dyj', array('id' => 1));
                    $rst = Dyj::ylydy($dyj2['api'], $dyj2['token'], $dyj2['yy_id'], $dyj2['mid'], $content);
                }
                //$this->getapi("" . $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&c=entry&a=wxapp&do=QtPrint2&m=zh_cjdianc&order_id=" . $order_id."&selectType=".$_GPC['selectType']); //打印机
    ...
    public function doPageQtPrint2($order_id,$selectType) {
        //前台打印
        global $_W, $_GPC;
        include IA_ROOT . '/addons/zh_cjdianc/print/dyj.php';
        $res = pdo_get('cjdc_order', array('id' => $order_id));
        ...
        if($selectType<3){
            ...
            $dyj = pdo_get('zh_jdgjb_dyj', array('id' => $_GPC['selectType']));
        }else if($selectType==3){
            $dyj = pdo_get('zh_cjdc_dyj', array('id' => $_GPC['selectType']));
        }
    ...
    //支付
    public function doPagePaySecond() {
        global $_W, $_GPC;
        $orderInfo = pdo_get('cjdc_order', array('id' => $_GPC['order_id']));
        if ($orderInfo['type'] == 1 && $orderInfo['state'] == 1 && strtotime($orderInfo['time']) <= time() - 15 * 60) {
            return json_encode(array('msg' => '订单已超时，或已被取消', 'code' => 0), 320);exit();
        ...
        $res = pdo_get('cjdc_pay', array('uniacid' => $_W['uniacid']));
        $res2 = pdo_get('cjdc_system', array('uniacid' => 4));
        if ($res2['url_name']) {
            $res2['url_name'] = $res2['url_name'];
        } else {
            $res2['url_name'] = '餐饮小程序';
...
//店内
    ...
    //所有增品，检查是否有赠品，没有赠品的话就送赠品
    public function doPageTypeShop() {
        global $_W, $_GPC;

        $res1 = pdo_getall('cjdc_user_level_gift', array('userId' => $_GET['user_id'],'giftLevel' =>  $_GET["userLevel"]));

        if($res1 && count($res1)>0){
            echo "1";
        }else{
            $curDate = date("Y-m-d H:m:s");
            $dataArray = pdo_getall('cjdc_shop', array('giftLevel' =>  $_GET["userLevel"],"status" =>1));
            if($dataArray) {
                for ($i = 0; $i < count($dataArray); $i++) {
                    $data['start_time']=$dataArray[$i]['start_time'];
                    $data['end_time']=$dataArray[$i]['end_time']?$dataArray[$i]['end_time']:"";

                    $strLength = strlen( $data['end_time']);
                    if($strLength<11){
                    ...
                    $endLongNum = strtotime($data['end_time']);
                    $curDateNum = strtotime($curDate);
                    if($endLongNum-$curDateNum>0){
                        $data2['giftId']=$dataArray[$i]['id'];
                        $data2['giftLevel'] = $dataArray[$i]['giftLevel'];
    ...
    //餐桌列表
    public function doPageGetDeskList() {
        global $_W, $_GPC;
        //$res = pdo_getall('cjdc_user_table', array('status' => 1,'giftLevel' =>  $_GET["userLevel"]));
        //$sql = "select * from " . tablename("cjdc_user_table") . " where status = 1 or status = 3";
        $dataArray = pdo_getall('cjdc_table', array());
    ...
    //查询用户id是否存在
    public function doPageGetUserById() {
        global $_W, $_GPC;
        //$res = pdo_getall('cjdc_user_table', array('status' => 1,'giftLevel' =>  $_GET["userLevel"]));
        //$sql = "select * from " . tablename("cjdc_user_table") . " where status = 1 or status = 3";
        $dataBean = pdo_get('cjdc_user', array('id' => $_GET['userId']));
        if(!$dataBean){
            $dataBean = pdo_get('cjdc_user', array('user_tel' => $_GET['userId']));
        }
        // $list = pdo_fetchall($sql);
        echo json_encode($dataBean);
    }


    //获取所有赠品,含非会员赠品和会员赠品
    public function doPageGetAllMyGift() {
        global $_W, $_GPC;
        //$res = pdo_getall('cjdc_user_level_gift', array('userId' => $_GET['user_id'],'isUsed' => 1));
        $sql = "select a.*,b.logo,b.end_time from " . tablename("cjdc_user_level_gift") . " a" . " left join " . tablename("cjdc_shop") . " b on b.id=a.giftId where a.userId='". $_GET['user_id']."' and a.isUsed=1  ORDER BY end_time desc;";
        $list = pdo_fetchall($sql);
        for ($i = 0; $i < count($list); $i++) {
            $time = strtotime($list[$i]['end_time']) - time();
            $list[$i]['isOverEnd'] = 0;
            if($time<0){
                $list[$i]['isOverEnd'] = 1;
            }
    ...
    //获取已使用的赠品
    public function doPageGetMyGiftUsed() {
        global $_W, $_GPC;
        //$res = pdo_getall('cjdc_user_level_gift', array('userId' => $_GET['user_id'],'isUsed' => 1));
        $sql = "select a.*,b.* from " . tablename("cjdc_user_level_gift") . " a" . " left join " . tablename("cjdc_shop") . " b on b.id=a.giftId where a.userId='". $_GET['user_id']."' and a.isUsed=2";
        $res = pdo_fetchall($sql);
        echo json_encode($res);
    }
    ...
    //消费赠品
    //消费赠品
    public function doPageDoMyGift() {
        global $_W, $_GPC;
        include IA_ROOT . '/addons/zh_cjdianc/print/dyj.php';
        //$res1 = pdo_update('cjdc_user_level_gift', array('isUsed' => 2), array('id' =>$_GET["id"]));
        $roomNote = $_GPC['note'];
        $res1 = pdo_update('cjdc_user_level_gift', array('isUsed' => 2,'roomNote'=>$roomNote), array('id' => $_GPC['id']));

        $dataBean = pdo_get('cjdc_user_level_gift', array('id' => $_GPC['id']));

        $dataGift = pdo_get('cjdc_shop', array('id' => $dataBean['giftId']));


        if($dataGift){
            $giftName = $dataBean['giftName'];
        }else{
            $giftName = "随机礼品";
        }

        $selectType = $dataGift['giftAddress'];
        ...
        $content .= "           免费礼品" . $style;
        $content .= "--------------------------------" . $style;
        $content .= "礼品名称：  ".$giftName . $style;
        $content .= "--------------------------------" . $style;
        $content .= "金额    ：0元" . $style;
        $content .= "--------------------------------" . $style;
        $content .= "领取人  ：" . $user['name'] . $style;
        $content .= "--------------------------------" . $style;
        $content .= "付款方式  ：" . $is_yue . $style;
        $content .= "--------------------------------" . $style;
        $content .= "商家名称：杰豪皇庭酒店" . $style;
        $content .= "--------------------------------" . $style;
        $content .= "领取时间：" .$dataBean["create_time"] . $style;
        $content .= "--------------------------------" . $style;
        $content .= "消费时间：" .date("Y-m-d H:i:s") . $style;
        $content .= "--------------------------------" . $style;
        $content .= "礼品结束时间：" .$dataGift["end_time"] . $style;
        ...
        $content .= "              收银台" . $style . $style . $style;
        $content .= "--------------------------------" . $style;
        $content .= "       免费礼品订单" . $style;
        $content .= "--------------------------------" . $style;
        $content .= "礼品名  ：".$dataBean['giftName'] . $style;
        $content .= "--------------------------------" . $style;
        $content .= "金额    ：0元" . $style;
        $content .= "--------------------------------" . $style;
        $content .= "付款人  ：" . $user['name'] . $style;
        $content .= "--------------------------------" . $style;
        $content .= "手机号：" . $user['user_tel'] . $style;
        $content .= "--------------------------------" . $style;
        $content .= "用户备注：" . $roomNote . $style;

        $content .= "--------------------------------" . $style;
        $content .= "商家名称：杰豪皇庭酒店" . $style;
        $content .= "--------------------------------" . $style;
        $content .= "领取时间：".$nowTime  . $style;
        $content .= "--------------------------------" . $style;
        $content .= "到期时间：".$dataGift["end_time"] . $style;
        $content .= "           店内订单  " . $style . $style;
        ...
        if ($selectType < 3&&$selectType>0) {
            ...
            $value = pdo_get('cjdc_dyj', array('id' => 1));
            //$rst = Dyj::ylydy($dyj2['api'], $dyj2['token'], $dyj2['yy_id'], $dyj2['mid'], $content);
...
//循环领取红包
    ...
    public function doPageZhuohao() {
        ...
        $data['isKtvTable'] = $res['isKtvTable'];
        echo json_encode($data);
    }
    ...
    public function doPageQtPrint() {
        ...
        $resGo2 = pdo_get('cjdc_goods', array('id' => $res2[0]['dishes_id']));

        $res3 = pdo_getall('cjdc_dyj', array('store_id' => $res['store_id'], 'state' => 1, 'location' => 1,'isKtvDyj'=>$resGo2['isKtvGood']));
        if ($res['type'] == 2) {
...
//喜讯
            ...
            $store = pdo_get('cjdc_store', array('id' => $res['store_id']), 'name');
            $resGo = pdo_get('cjdc_goods', array('id' => $res2[0]['dishes_id']));
            $res3 = pdo_getall('cjdc_dyj', array('isKtvDyj'=>$resGo['isKtvGood'],'store_id' => $res['store_id'], 'state' => 1, 'location' => 2));
            if ($res['pay_type'] == 1) {
                $is_yue = "微信支付";
            } elseif ($res['pay_type'] == 2) {
                $is_yue = "余额支付";
...
//退款
    ...
    public function doPageJcPrint() {
        ...
        $sql = " select a.name,b.name,a.isKtvTable as type_name from " . tablename('cjdc_table') . " a left join " . tablename('cjdc_table_type') . " b on a.type_id=b.id where a.id={$res['table_id']}";
        $table = pdo_fetch($sql);

        $res3 = pdo_getall('cjdc_dyj', array('isKtvDyj'=>$table['isKtvTable'],'store_id' => $res['store_id'], 'state' => 1, 'location' => 1));
...
//喜讯
    ...
    public function doPageJcPrint2() {
        ...
        if ($type['print_type'] == 2) {
            ...
            foreach ($result as $key => $value) {
                $res3 = pdo_getall('cjdc_dyj', array('isKtvDyj'=>0,'store_id' => $res['store_id'], 'state' => 1, 'location' => 2, 'tag_id' => $key));
...
//喜讯
            ...
            $store = pdo_get('cjdc_store', array('id' => $res['store_id']), 'name');
            $res3 = pdo_getall('cjdc_dyj', array('isKtvDyj'=>0,'store_id' => $res['store_id'], 'state' => 1, 'location' => 2));
            $content = '';
            $content .= "         " . $store['name'] . "\n\n";
...
//退款成功
    ...
    public function doPageCheckRetail() {
        ...
        $res2 = pdo_get('cjdc_user', array('id' => $_GPC['user_id']));
        if($res2['user_tel']){
            $res3 = pdo_get('zh_jdgjb_distribution', array('user_tel' => $res2['user_tel'],'state'=>2));
            if($res3&&$res3['user_tel']){
    ...
    public function doPageMyDistribution() {
        global $_W, $_GPC;
        $res = pdo_get('cjdc_retail', array('user_id' => $_GPC['user_id'],'uniacid'=>'3'));
        echo json_encode($res);
    }
    ...
    /*
     * 更新分销数据，如果酒店分享二维码给其他人成为上下级关系，则默认在其他小程序也为上下级关系,酒店-》商城
     * 宝塔每天都执行该任务
     */
    public function doPageShowFxCenter(){
        global $_GPC, $_W;
        ...
        $res = pdo_get("cjdc_user", array("id" => $_GPC["id"]));
        if($res) {
            $mobile = $res['user_tel'];
            if($mobile) {
                //echo $mobile;
                $res2 = pdo_get('zh_jdgjb_distribution', array('user_tel' => $mobile, 'state' => 2));
                if ($res2) {
                    echo "1";
                } else {
                    echo "0";
...
//喜讯
    ...
    public function doPageRushPrint() {
        ...
        $store = pdo_get('cjdc_store', array('id' => $res['store_id']), 'name');
        $res3 = pdo_getall('cjdc_dyj', array('isKtvDyj'=>0,'store_id' => $res['store_id'], 'state' => 1, 'location' => 1));
